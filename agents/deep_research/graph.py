...
def ask_user_node(state: AgentState):
    ...
    if state.clarification and state.clarification.needed:
        ...
        user_input = input(
            f"Agent: {state.clarification.question} "
        )  # waiting for user
...
def generate_subqueries_node(state: AgentState):
    ...
    subqs = b.GenerateSubqueries(
        question=state.question, clarification_details=clarif_detail
    )
    # Ensure we have a list of strings (BAML returns a Python list for string[] output)
    state.subqueries = (
        list(subqs) if isinstance(subqs, list) else subqs.queries
    )  # .queries if wrapped in a model
    ...
    if state.plan:
        for step in state.plan.steps:
            ...
            if tool == "WebSearch":
                ...
                results.extend(
                    search_res
                )  # Extend directly as it's already list of dicts
            elif tool == "PriceLookup":
                price_str = get_current_price(query)
                if price_str:
                    # Format price result as a dict for consistency
                    results.append(
                        {"content": f"Current {query} price: {price_str}", "link": None}
                    )
                else:
                    results.append(
                        {
                            "content": f"Current {query} price: (unavailable)",
                            "link": None,
                        }
                    )
...
def filter_results_node(state: AgentState):
    ...
    print(
        f"LLM Filtered Results (Top {len(state.relevant_results)}): {state.relevant_results}"
    )  # Add some logging
...
def answer_node(state: AgentState):
    ...
    relevant_context_dicts: List[Dict[str, Optional[str]]] = (
        state.relevant_results or []
    )
...
def additional_search_node(state: AgentState):
    ...
    if new_info_results:
        ...
        existing_links = {
            res.get("link") for res in state.relevant_results if res.get("link")
        }
...
def build_agent_graph():
    ...
    graph_builder.set_entry_point(
        "clarify"
    )  # Use set_entry_point instead of add_edge from START
